...
- Les `Point` de l'Équipement sont déclarés sur la règle PROCEDURE (lots 082-083) :
  - colonnes cochées d'office (`SheetExtractionRule.UnconditionalColonneNames`) — profil standard : `"VISITE PRÉALABLE CHANTIER"`, `"AUTORISATION DÉPLATINAGES"`, `"AUTORISATION DE REMISE EN SERVICE"`, `"RÉCEPTION FINALE CHANTIER"` ;
  - règles conditionnelles (`PointRules`), évaluées sur chaque **vraie tâche** (ordre renseigné, titres de section exclus) : le point est créé si **au moins une** tâche satisfait la règle, sans avertissement sinon — profil standard : `"PROCÉDURE MAD"` si une tâche a le type `MAD`, `"PROCÉDURE REL"` si une tâche a le type `REL` (champ `TypeTacheMultipleAlias`, colonne R).
  - Demande du client du 19/09 (verbatim dans le ticket 083). Décision du 19/09 (lot 082, `tickets-tdd-lot-082-points-office-element-parent-procedure.md`) : jusque-là, le code créait un point nommé d'après chaque Tableau, ce qui mélangeait les deux notions ; TRAVAUX COMPLET/TRAVAUX DETAIL sont des tableaux seulement.
