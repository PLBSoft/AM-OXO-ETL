...
### A2. Modèle pivot
**Tests d'abord** : construction de `EquipementPivot`, `IsolementPivot`, `PointPivot`, `TacheMultiplePivot`, `ExtractionError`/`ExtractionErrorCode`, `ImportResult` (dont `HasErrors`).
...
**Tests d'abord** : `Equals`/`NotEquals` sur une valeur de champ déjà extraite ; liste vide de règles = toujours créer le Point ; valeur ne matchant aucune règle = pas de Point créé + avertissement non bloquant ajouté à `ImportResult.Errors` (§3.2 du modèle de domaine — bien vérifier que ce n'est **pas** un rejet de bloc).
...
- Alias `TypeTacheMultiple.Code` (`R9` → `TM_PROC_MAD`/`TM_PROC_REL`)
...
### C2. ISOLEMENT
**Tests d'abord** : pas=7, repère composé `{K6:T6}-{Identification}`, Points `PROLOCK VANNES`/`DEPROLOCK VANNES`/`ZÉRO ENERGIE...` (dernier conditionné à `TypeElement = ZERO ENERGIE`).
 
### C3. PLATINES
**Tests d'abord** : pas=8, 7 Colonnes de Points (dont les variantes `DEBUT` uniquement — **vérifier explicitement dans un test qu'aucun Point `FIN...` n'est généré**, pour matérialiser la décision client actée et éviter une régression silencieuse si quelqu'un rajoute la feuille FIN plus tard sans relire la spec).
 
### C4. ORIFICES CAPACITES
**Tests d'abord** : pas=8, mêmes 4 Colonnes de Points systématiques (pas de condition).
 
### C5. AUTRES JOINTS TOUCHES
**Tests d'abord** : pas=7 (confirmé, pas 3), condition `POSE ÉTIQUETTES` créé seulement si `TypeElement ≠ "TUBING"`.
 
### C6. DIVERS
**Tests d'abord** : pas=3, `loc1` lu en `B6:E6` (valeur brute, pas de transformation), 4 conditions de Points sur `TypeElement` (`INSTRUMENTATION`/`ZERO ENERGIE`/`SOUPAPE` ×2/`POINT FEU` ×3) — bien utiliser la littérale `"POINT FEU"` (pas `"POINT DE FEU"`) conformément à la décision client.
