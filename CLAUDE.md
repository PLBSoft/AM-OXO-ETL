...
- Next up per the ticket doc: Lot B (Application-layer generic engine — `IWorkbookReader`, `TextTransform`/`RepeatingBlockLocator`/`ConditionalPointRule` evaluators), then Lot C (one extraction service per sheet), then Lot D (orchestrator + integration tests against the real fixtures).
