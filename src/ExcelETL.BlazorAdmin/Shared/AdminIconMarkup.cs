...
public static class AdminIconMarkup
{
    ...
    // Dark-mode toggle (NavMenu.razor sidebar footer). No existing sun/moon shape in this project
    // to reuse verbatim -- adapted from Feather Icons' "sun"/"moon" glyphs (MIT license, simple
    // stroke-based shapes), chosen over a hand-transcribed Bootstrap Icons path for the same
    // "avoid transcription risk on a complex glyph" reason as Key/Clock above.
    ...
    // 2026-09-16: "reset the standard profile to its seed definition" row action
    // (ImportProfiles.razor/ExportProfiles.razor). No existing shape in this project's icon set to
    // reuse verbatim -- a deliberately simple, hand-drawn circular arrow (open arc + arrowhead),
    ...
    // Lot 078.10: "View details" row action (read-only plain-language view of an import profile) --
    // the Bootstrap Icons bi-eye shape.
