...
// - what PROCEDURE ignores: colour settings;
...
{
    public static SheetRuleIgnoredSettings None { get; } = new(false, false, false, false);
    ...
    public static SheetRuleIgnoredSettings For(SheetExtractionRuleDraft rule)
    {
        ...
        return new SheetRuleIgnoredSettings(
            SheetNotProcessed: false, couleurIgnored, defaultIgnoredBecauseCell, allowedIgnoredWithoutCell)
        {
            AnyConfiguredSettingIgnored = couleurIgnored || defaultIgnoredBecauseCell || allowedIgnoredWithoutCell
        };
    }
}
