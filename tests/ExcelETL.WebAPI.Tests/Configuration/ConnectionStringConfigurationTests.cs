...
namespace ExcelETL.WebAPI.Tests.Configuration;
...
public class ConnectionStringConfigurationTests
{
    ...
    private static string LoadDefaultConnection(string fileName)
    {
        var configuration = new ConfigurationBuilder()
            .AddJsonFile(WebApiSettingsPath(fileName), optional: false)
            .Build();
    ...
    private static string WebApiSettingsPath(string fileName)
    {
        ...
        return Path.Combine(directory.FullName, "src", "ExcelETL.WebAPI", fileName);
    }
}
